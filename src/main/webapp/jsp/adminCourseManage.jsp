...
<script type="text/javascript">
    function screenStu(pdata1,pdata2){
        layer.confirm('确定要对学生随机筛选吗？', {
            btn: ['确认','取消'] //按钮
        }, function(){
            $.ajax({
                url:'screenStu?courseId='+pdata1+'&stuCountStr='+pdata2,
                type:'post',
            });
...
<section class="layui-larry-box">
	...
	<blockquote class="layui-elem-quote news_search">
		...
		<form action="adminCourseManage" method="post">
			...
			<div class="layui-inline">
				<a onclick="openCourse()" class="layui-btn layui-btn-danger batchDel">开放选课</a>
			</div>
			</c:if>
			<c:if test="${openFlag != 'false'}">
				<div class="layui-inline">
					<a onclick="closeCourse()" class="layui-btn layui-btn-danger batchDel">禁止选课</a>
				</div>
			</c:if>
		</form>
					...
					<tbody class="news_content">
						...
						<tr>
							...
							<td>
								...
								<a onclick="screenStu(${list.id},${list.stuCount})" class="layui-btn layui-btn-normal layui-btn-mini news_collect"><i class="layui-icon"></i> 随机筛选</a>
							</td>
						</tr>
						</c:forEach>
					</tbody>
		...
		<form name="form1" method="post" action="adminCourseManage">
